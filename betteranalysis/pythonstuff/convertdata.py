...
# Load CSVs
proPaths = [
    "betteranalysis/good_angles.csv"
]
testPath = "Script\\dataAnalysis\\media\\test_angles.csv"
...
# Save mappings
with open("script/outputcsv/pro_multivariate_dtw_frame_mappings.json", "w") as f:
    json.dump(pro_dtw_mappings, f, indent=2)
with open("script/outputcsv/test_multivariate_dtw_frame_mapping.json", "w") as f:
    json.dump(test_mapping, f, indent=2)

# Save aligned data
pro_avg_df.insert(0, "frame", range(len(pro_avg_df)))
pro_avg_df.to_csv("script/outputcsv/donny_pro_multivariate_dtw_aligned_avg.csv", index=False)
test_aligned_df.insert(0, "frame", range(len(test_aligned_df)))
test_aligned_df.to_csv("script/outputcsv/test_multivariate_dtw_aligned.csv", index=False)
...
with open("script/outputcsv/test_stage_frame_indices_full.json", "w") as f:
...
os.makedirs("Script/outputcsv/test_stages_full", exist_ok=True)

writers = {}
for stage in stage_to_full_gordon_frames:
    out_path = f"Script/outputcsv/test_stages_full/test_{stage}.mp4"
