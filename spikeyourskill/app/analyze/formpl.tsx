  ...
  return (
    <main>
      <div className="flex flex-col items-center mb-4">
            <h1 className = "font-ubuntu text-4xl font-bold text-blue-800">Input Video</h1>
      </div>
        <form onSubmit={handleSubmit} className="flex flex-col text-center justify-center">
        <div>
                {vid && (
                    <p className="mb-4">Selected Video: {vid.name}</p>
                )}
          <input
            type="file"
            accept="video/*"
      ...
      <div className="justify-evenly flex gap-4 mt-4">
        <p>Video Length: {result} seconds</p>
      ...
      {error && <p className = "text-center mt-4 font-ubuntu text-4xl font-bold text-red-800">{error}</p>}
    </main>
  );
}